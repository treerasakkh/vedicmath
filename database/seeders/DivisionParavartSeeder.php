...
use App\models\DivisionParavart as CurrentModel;

class DivisionParavartSeeder extends Seeder
{
    /**
    ...
    protected function factory(string $level, string $difficulty, int $numItems): array
    {
        ...
        $divParavart = new DivisionParavart($level, $difficulty);
        $quizzes = collect($divParavart->get($numItems));
        
        $quizzes = $quizzes->map(function ($quiz) use ($levelId, $difficultyId) {
            return [
                'number1' => $quiz->dividend,
                'number2' => $quiz->divisor,
                'level_id'=>$levelId,
