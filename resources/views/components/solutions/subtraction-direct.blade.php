<x-frame class="p-16">
    ...
    @if ($showSolution == true)
        <table class="mt-12">
            <tr class="relative after:absolute after:content-['-'] after:top-4">{!! $row1 !!} </tr>
            <tr class="border-b border-b-gray-400">{!! $row2 !!}</tr>
            <tr class="text-red-500">{!! $row3 !!}</tr>
            <tr class="border-t border-b-4 border-double border-gray-400 *:w-5 font-semibold">{!! $row4 !!}</tr>
        </table>
    @else
        <table class="mt-12">
            <tr class="relative after:absolute after:content-['-'] after:top-4">{!! $row1 !!} </tr>
